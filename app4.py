...
with tab2:
    ...
    with st.form("supply_form"):
        ...
        with col2:
            c_email = st.text_input("聯絡 Email")
            c_region = st.selectbox("縣市", cities_list, key="c_city")
        st.markdown("---")
        c_drug = st.selectbox("有貨藥品", df_drugs["藥品名稱"].tolist(), key="c_drug")
        c_conditions = st.multiselect("給付條件", ["健保", "自費", "國健署專案"])
        
        if st.form_submit_button("📤 提交"):
            if submit_supply(c_code, c_name, c_region, c_drug, c_conditions, c_email):
                st.success("提交成功，待審核後上架。")
